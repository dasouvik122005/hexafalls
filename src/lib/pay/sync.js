...
export async function reconcilePayments(db, { origin = "" } = {}) {
  ...
  for (const p of pending) {
    ...
    try {
      ...
      if (await markPaid(db, p.id, entId)) {
        summary.markedPaid++;
        // Per-member side-effects (member + admin notif, buyer email). Only on
        // the real flip; best-effort so a notify failure doesn't abort the run.
        try {
          ...
          summary.errors.push(`notify ${p.id}: ${e?.message ?? e}`);
        }
      }
    } catch (e) {
...
// Reconcile only ONE user's not-yet-paid payments against Elixpo Pay. Same flip
// + rollup behaviour as reconcilePayments, scoped to a single user so it's cheap
// enough to run inline when the owner views their own teams page (a webhook may
// not have landed yet right after checkout). Best-effort; returns a summary.
export async function reconcileUserPayments(db, { userId, origin = "" } = {}) {
  const summary = { scanned: 0, markedPaid: 0, squadsSettled: 0, errors: [] };
  ...
  const pendingRes = await db
    .prepare(
      `SELECT id, user_id, squad_id, event FROM payments
