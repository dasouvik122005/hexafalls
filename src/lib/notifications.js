...
// Fan a notification out to every admin / organizer (for ops-facing events like
// a member paying their fee). Best-effort; never throws into the caller.
export async function notifyAdmins(payload, { db = getDB() } = {}) {
  try {
    const r = await db
      .prepare(`SELECT id FROM users WHERE role IN ('admin','organizer')`)
      .all();
    const ids = (r.results ?? []).map((u) => u.id);
    return notifyMany(ids, payload, { db });
  } catch {
    return [];
  }
}
