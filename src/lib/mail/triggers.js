// The events that send email (everything else → in-profile notifications). Each
// maps to its own Elixpo Mails template webhook, set in .env.local. A blank
// per-template key falls back to the shared ELIXPO_MAILS_ENDPOINT_KEY; blank
// everywhere → the send is skipped (logged).
// Buyer payment confirmation is sent here via `payment_received` — the provider
// (Razorpay/Elixpo Pay) receipt to the buyer is suppressed (see checkout).
...
const WEBHOOK_ENV = {
  ...
  payment_received: "ELIXPO_MAILS_WEBHOOK_PAYMENT_RECEIVED",
};
...
// payment_received — buyer's own payment confirmation, sent to the member as
// soon as THEIR fee is recorded paid (replaces the provider receipt).
export function sendPaymentReceived({ to, name, teamName, event, amount, teamUrl, idempotencyKey }) {
  return sendMail({
    endpointKey: endpointFor("payment_received"),
    to,
    variables: {
      name: name ?? "there",
      team_name: teamName ?? "",
