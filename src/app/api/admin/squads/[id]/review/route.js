...
export async function POST(req, { params }) {
  ...
  // Clearing review SHORTLISTS the team. Paid events then owe the entry fee
  // (leader pays → approved). Free events have no fee, so review-pass = approved.
  const paid = isPaidEvent(squad.event);
  const nextStatus =
    decision === "reject" ? "rejected" : paid ? "shortlisted" : "approved";
  ...
  try {
    ...
    if (decision === "approve" && nextStatus === "shortlisted") {
      // Paid event shortlisted — tell the team to pay; the "approved" email
      // fires later from the payment settle (payment_complete).
      await notifyMany(
        members.map((m) => m.id),
        {
          kind: "team_shortlisted",
          title: `${squad.name} is shortlisted`,
          body: `Your ${eventLabel} team cleared review. The leader must pay the entry fee to lock in your seats.`,
          link: teamPath,
        },
        { db },
      );
    } else if (decision === "approve") {
      // Free event — review-pass means fully approved.
      await notifyMany(
        members.map((m) => m.id),
        {
          kind: "team_approved",
