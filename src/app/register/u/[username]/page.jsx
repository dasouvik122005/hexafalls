// /register/u/[username] — public hacker profile.
//
// SEO-friendly: title + description derived from username + bio.
// Renders: identity card, bio, links, the squads this hacker belongs to.
...
export default async function HackerProfilePage({ params }) {
  const { username } = await params;
  const db = getDB();
        ...
        <div className="flex flex-col gap-6">
          ...
          {(user.github || user.linkedin || user.portfolio) && (
            <div className="flex flex-wrap gap-2">
              ...
              {user.portfolio && (
                <ExtLink href={user.portfolio} label="Portfolio" />
              )}
            </div>
          )}
