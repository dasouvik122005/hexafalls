/* ═══════════════════════════════════════════
   HexaFalls — Software Track Registration JS
   ═══════════════════════════════════════════ */
...
/* ── Particles ── */
const pColors = ['#c9a84c','#00e5ff','#a78bfa'];
for (let i = 0; i < 20; i++) {
...
document.addEventListener('mousemove', e => {
  ...
  if (now - lastTrail > 40) {
    ...
    trail.style.cssText = `left:${e.clientX-2}px;top:${e.clientY-2}px;background:${Math.random()>.5?'#c9a84c':'#00e5ff'};box-shadow:0 0 6px ${Math.random()>.5?'#c9a84c':'#00e5ff'};`;
...
function lookupHexaId(idx) {
  ...
  setTimeout(() => {
    ...
    if (found) {
      ...
      nameInput.style.borderColor = 'var(--cyan-dim)';
      emailInput.style.borderColor = 'var(--cyan-dim)';
...
function showPanel(n) {
  ...
  // Render members when entering step 2
  ...
  if (n > currentStep) {
    ...
    if (currentStep === 1) {
      ...
      if (![...radios].some(r => r.checked)) {
        valid = false;
        alert('Please select your hackathon experience.');
      }
      if (!document.getElementById('teamSize').value) {
        valid = false;
        document.getElementById('teamSize').style.borderColor = 'var(--danger)';
      }
    }
    if (!valid) return;
  }
...
function buildSummary() {
  ...
  for (let i = 1; i <= n; i++) {
    ...
    membersHtml += `<div style="margin-left:16px;margin-bottom:6px"><i class="bi bi-person-fill" style="color:var(--cyan)"></i> <strong>${name}</strong> · ${email} · T-Shirt: ${tshirt} · Food: ${food}${github !== '—' ? ' · <a href="'+github+'" target="_blank" style="color:var(--cyan)">GitHub</a>' : ''}${linkedin !== '—' ? ' · <a href="'+linkedin+'" target="_blank" style="color:var(--cyan)">LinkedIn</a>' : ''}</div>`;
  }
  ...
  document.getElementById('summaryBox').innerHTML = `
    ...
    <div style="margin-bottom:12px"><i class="bi bi-braces" style="color:var(--gold)"></i> <strong>Tech Stack:</strong> ${techs}</div>
...
function handleSubmit() {
  ...
  console.log('HexaFalls Registration:', data);

  const btn = document.getElementById('submitBtn');
  btn.innerHTML = '<i class="bi bi-check-circle-fill"></i> Scroll Sealed — Owls Dispatched';
  btn.style.background = 'linear-gradient(135deg,#00e5ff,#00b4cc)';
...
function spawnConfetti() {
  ...
  const colors = ['#c9a84c', '#00e5ff', '#f0d080', '#a78bfa', '#ff6b9d'];
  el.style.cssText = `
    position:fixed;
...
confettiStyle.textContent = `
  @keyframes confettiFall {
    ...
    100% { transform: translateY(${window.innerHeight}px) translateX(${-100 + Math.random()*200}px) rotate(${360+Math.random()*360}deg); opacity:0; }
  }`;
